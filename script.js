function calcular() {
  const getInput = (id, isFloat = false) => {
    const valor = document.getElementById(id).value.replace(",", ".");
    return isFloat ? parseFloat(valor) : parseInt(valor);
  };
  ...
  var tamanhoHorizontalPlaca = parseFloat(
    document.getElementById("tamanhoHorizontalPlaca").value
  );
  var tamanhoVerticalPlaca = parseFloat(
    document.getElementById("tamanhoVerticalPlaca").value
  );
  var pixelsHorizontal = parseInt(
    document.getElementById("pixelsHorizontal").value
  );
  var pixelsVertical = parseInt(
    document.getElementById("pixelsVertical").value
  );
  ...
  document.getElementById("resultadoTamanhoPainel").textContent =
    "Tamanho do painel: " + tamanhoPainel + "m²";
  document.getElementById("resultadoTamanhoPlacas").textContent =
    "Quantidade de placas: " + tamanhoTotalPlacas;
  document.getElementById("resultadoResolucaoPixels").textContent =
    "Resolução pixels: " + totalPixelsHorizontal + "x" + totalPixelsVertical;
